...
async def cmd_start(message: Message):
    ...
    text = (
        "🤖 **Панель Управления ИИ-Агентом**\n\n"
        "Здесь вы можете управлять состоянием работы ИИ-агента для вашего личного Telegram-аккаунта.\n\n"
        "• **Тумблер**: Включает или отключает автоответы.\n"
        "• **Белый список**: ИИ отвечает ТОЛЬКО в выбранных чатах.\n"
        "• **Личность (Persona)**: Промпт и стиль общения бота."
    )
    await message.answer(text, reply_markup=get_main_menu_keyboard(is_enabled), parse_mode="Markdown")
...
async def cb_main_menu(call: CallbackQuery):
    ...
    await call.message.edit_text(text, reply_markup=get_main_menu_keyboard(is_enabled), parse_mode="Markdown")
    await call.answer()
...
async def cb_menu_status(call: CallbackQuery):
    ...
    text = (
        "ℹ️ **Текущий Статус Системы**\n\n"
        f"• **Статус ИИ-Агента**: {status_icon}\n"
        f"• **Активная Личность**: {persona_name}\n"
        f"• **Чатов в Белом списке**: {len(active_chats)}\n"
        f"• **Используемая модель**: `{settings.GEMINI_MODEL}`\n\n"
        "Система работает вштатном режиме."
    )
